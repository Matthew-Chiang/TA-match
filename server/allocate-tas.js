...
async function allocateTAs(semester = month + year, preferenceWeighting = 1) {
    ...
    for (courseCode of allocationOrder) {
        ...
        for (applicant of coursesObj[courseCode]) {
            ...
            if (!allocationObj[courseCode]) {
                allocationObj[courseCode] = [
                    {
                        email: applicant.email,
                        hours: allocatedHours,
                        name: applicant.name,
                        fundability: applicant.status,
                    },
                ];
            } else {
                allocationObj[courseCode].push({
                    email: applicant.email,
                    hours: allocatedHours,
                    name: applicant.name,
                    fundability: applicant.status,
    ...
    for (courseCode of Object.keys(allocationObj)) {
        let allocationCol = db.collection(
            `/courses/${semester}/courses/${courseCode}/allocation`
        );
        ...
        const profEmail = profDoc.data().instructor;
        ...
        notificationsCollection
            ...
            .set({
                title: "You have been matched! Course Code: " + courseCode,
                text: "Accept/Reject TA allocations before the deadline",
            });
    }
}
...
async function buildCoursesObj(semester, preferenceWeighting) {
    ...
    for (course of coursesList) {
        let applicants = await db
            .collection(`/courses/${semester}/courses/${course}/applicants`)
            .get();
        ...
        applicants.forEach((applicant) => {
            ...
            if (priortyRank == "profRanking") {
                tempApplicant.realRanking = parseInt(
                    tempApplicant.ProfRanking,
                    10
                );
            }
        ...
        if (priortyRank == "taRanking") {
            applicantsList.sort(
                (a, b) =>
                    a.taRanking - b.taRanking || a.ProfRanking - b.ProfRanking
            );
        ...
        for (applicant of applicantsList) {
            applicant.realRanking =
                applicant.realRanking + numApplicants * (applicant.status - 1);
        }
